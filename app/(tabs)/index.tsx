...
const onRefresh = useCallback(async () => {
  ...
  try {
    ...
    const updatedTokens = allTokens.map(token => {
      const changePercent = (Math.random() * (3 - 0.002) + 0.002) * (Math.random() > 0.5 ? 1 : -1);
...
const portfolioStats = useMemo(() => {
  ...
  for (const token of tokens) {
    ...
    const value = price * amount * (1 + token.profitPercent / 100);
  ...
  const profitPercent = totalValue !== 0 ? (totalChange / (totalValue - totalChange)) * 100 : 0;
  ...
  return {
    ...
    totalChange: `${totalChange >= 0 ? '+' : ''}${totalChange.toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    })}`,
    profitPercent: `${profitPercent >= 0 ? '+' : ''}${profitPercent.toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    })}%`,
    profitColor:
      profitPercent > 0 ? '#4caf50' : profitPercent < 0 ? '#ff6666' : '#999999',
  };
}, [tokens]);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <View style={styles.balanceContainer}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: hp('0%') }}>
            <Text style={[styles.subBalance, { color: portfolioStats.profitColor }]}>
  ...
  const profitColor =
    token.profitPercent > 0
      ...
      ? '#ff6666'
      : '#999999';
  const profit = (token.profitPercent > 0 ? '+' : '') + token.profitPercent.toFixed(2) + '%';
  const amount = `${token.amount} ${token.symbol.toUpperCase()}`;
const amountFormatted = `${formatAmount(token.amount)} ${token.symbol.toUpperCase()}`;

  return (
    <View key={token.id} style={[styles.Notification, index !== 0 && { marginTop: -10 }]}>
      <View style={styles.tokenRow}>
        <View style={{ position: 'relative' }}>
          <Image source={{ uri: token.logo }} style={styles.tokenLogo} resizeMode="cover" />
        ...
        <View style={styles.tokenPrice}>
          <Text style={styles.tokenPriceValue}>{token.totalValue}</Text>
          <Text style={[styles.tokenProfit, { color: profitColor }]}>{profit}</Text>
        </View>
      </View>
    </View>
  );
